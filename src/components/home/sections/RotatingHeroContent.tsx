...
const RotatingHeroContent = ({ onShowAuth }: RotatingHeroContentProps) => {
  ...
  return (
    <div className="relative overflow-hidden px-2 py-4 md:px-6 md:py-12 lg:px-8 lg:py-16">
      ...
      <div className="relative z-10 text-center max-w-4xl mx-auto space-y-4 md:space-y-8 lg:space-y-12">
        {/* Icon */}
        <div className="mb-2 md:mb-4 lg:mb-6">
          <div className={`w-14 h-14 md:w-16 md:h-16 mx-auto rounded-full bg-gradient-to-r ${currentContent.gradient} flex items-center justify-center transition-all duration-600 shadow-lg`}>
            <CurrentIcon className="h-7 w-7 md:h-8 md:w-8 text-white" />
          </div>
        </div>

        {/* Carousel container with overflow hidden */}
        <div className="relative min-h-[420px] sm:min-h-[450px] md:min-h-[500px] overflow-hidden">
          <div 
              ...
              return (
                ...
                >
                  <div className="space-y-3 sm:space-y-4 md:space-y-6 max-w-4xl mx-auto">
                    <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-foreground leading-tight text-center px-2">
                      {t(`hero.${content.slideKey}.title`)}
                    </h2>
                    
                    <h3 className={`text-lg sm:text-xl md:text-2xl lg:text-3xl font-medium text-transparent bg-clip-text bg-gradient-to-r ${content.gradient} text-center px-2`}>
                      {t(`hero.${content.slideKey}.subtitle`)}
                    </h3>
                    
                    <p className="text-sm sm:text-base md:text-lg lg:text-xl text-muted-foreground max-w-3xl mx-auto text-center px-3 sm:px-4 md:px-6 leading-relaxed">
                      {t(`hero.${content.slideKey}.description`)}
                    </p>

                    {/* Examples for main slide */}
                    {content.showExamples && (
                      <div className="flex flex-wrap gap-2 justify-center mt-3 sm:mt-4 md:mt-6 px-2">
                        <Badge variant="secondary" className="px-3 py-1.5 text-xs sm:text-sm md:text-base whitespace-nowrap">
                     ...
                     {/* Action buttons */}
                    <div className="flex flex-col sm:flex-row gap-3 justify-center mt-5 sm:mt-6 md:mt-8 px-2">
                      <Button 
                        size="lg" 
        ...
        <div className="flex justify-center mt-5 sm:mt-6 md:mt-8 gap-2 sm:gap-3">
          {heroContent.map((_, index) => (
            <button
              key={index}
              onClick={() => navigateToSlide(index)}
              disabled={isTransitioning}
              className={`min-w-12 min-h-12 rounded-full transition-all duration-300 active:scale-90 touch-manipulation ${
